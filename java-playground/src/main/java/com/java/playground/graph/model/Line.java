 ...
 */
public final class Line<T extends Comparable<T>> implements Edge<T> {
  private final String id;
  private final Node<T> source;
  private final Node<T> sink;
  private final Optional<Double> weight;

  private Line(Node<T> source, Node<T> sink, Optional<Double> weight) {
  ...
  public static <T extends Comparable<T>> Line<T> of(Node<T> source, Node<T> sink, double weight) {
    return new Line<>(source, sink, Optional.of(weight));
  }

  public static <T extends Comparable<T>> Line<T> of(T source, T sink, double weight) {
    return of(Node.of(source), Node.of(sink), weight);
  }

  public static <T extends Comparable<T>> Line<T> of(Node<T> source, Node<T> sink) {
    return new Line<>(source, sink, Optional.empty());
  }

  public static <T extends Comparable<T>> Line<T> of(T source, T sink) {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(id, ImmutableSet.of(source, sink), weight);
  }
}
